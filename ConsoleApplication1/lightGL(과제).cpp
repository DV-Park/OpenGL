...
void MyLightInit()	
{
	...
	glLightModeli(GL_LIGHT_MODEL_LOCAL_VIEWER, GL_FALSE);	// default (시점위치)
	glLightModeli(GL_LIGHT_MODEL_TWO_SIDE, GL_FALSE);		// default (양면조명)

	// 0번 광원
	GLfloat light0_ambient[]={Ia, 0.0, 0.0, 1.0}; // 0이면 벡터 1이면 점 (위치성 광원, 방향성 광원)
...
void MyMaterialInit()
{
	...
	//glMaterialfv(GL_FRONT_AND_BACK, GL_EMISSION, material_emission); // 발광색
